...
{
	private void testTemplate(JSONValue val, String expected)
	{
	...
	void testArrayWrite()
	{
		JSONValue[] simple = { JSONBooleanValue.TRUE, null, new JSONStringValue("Testing"), JSONNullValue.NULL, new JSONNumberValue(-34.2) };
		JSONArrayValue empty = new JSONArrayValue(JSONArrayValue.EMPTY);
		JSONArrayValue simVal = new JSONArrayValue(simple);
		JSONObject obj = new JSONObject();
		obj.setField("text", "Test");
		obj.setField("empty", "");
		obj.setField("arr", simple);
		obj.setField("bool", false);
		JSONValue[] inner = { new JSONStringValue("This"), new JSONStringValue("is"), new JSONStringValue("inner"), new JSONStringValue("array") };
		
		JSONValue[] complex = { simVal, null, new JSONObjectValue(obj), new JSONNumberValue(12),
				new JSONArrayValue(inner), empty, JSONBooleanValue.TRUE };
		
		JSONArrayValue comVal = new JSONArrayValue(complex);
		testTemplate(empty, "[]");
